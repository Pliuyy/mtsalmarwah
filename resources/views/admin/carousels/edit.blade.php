...
<div class="px-6 py-10 min-h-screen bg-gray-50">
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8">
        <h1 class="text-3xl font-bold text-navy-blue mb-6">Edit Slide Carousel</h1>
        <a href="{{ route('admin.carousels.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 text-sm px-3 py-2 rounded transition duration-300" style="position: relative; bottom: -14px; left: 225px;">
            ←
        </a>
        ...
        <form action="{{ route('admin.carousels.update', $carousel->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="{{ old('title', $carousel->title) }}" class="w-full border-gray-300 rounded px-3 py-2">
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type-select" class="w-full border-gray-300 rounded px-3 py-2">
                    <option value="image" {{ old('type', $carousel->type) === 'image' ? 'selected' : '' }}>Gambar</option>
                    <option value="video" {{ old('type', $carousel->type) === 'video' ? 'selected' : '' }}>Video</option>
                </select>
            </div>
            ...
            <div id="image-input" class="mb-4 hidden">
                ...
                @if($carousel->type === 'image' && $carousel->image_path)
                <p class="text-sm text-gray-600 mt-2">Gambar saat ini:</p>
                <img src="{{ asset('storage/' . $carousel->image_path) }}" alt="Current Image"
            ...
            <div id="video-options" class="mb-4 hidden">
                ...
                <select name="video_source" id="video-source-select" class="w-full border-gray-300 rounded px-3 py-2 mb-3">
                    <option value="youtube" {{ old('video_source', $carousel->video_url ? 'youtube' : 'upload') === 'youtube' ? 'selected' : '' }}>YouTube</option>
                ...
                <div id="youtube-input" class="mb-3 hidden">
                    ...
                    <input type="text" name="video_url" value="{{ old('video_url', $carousel->video_url) }}" class="w-full border-gray-300 rounded px-3 py-2">
                    @if($carousel->type === 'video' && $carousel->video_url)
                    <p class="text-sm text-gray-600 mt-2">Video saat ini:</p>
                    <img src="https://img.youtube.com/vi/{{ $carousel->video_url }}/hqdefault.jpg"
                        alt="Current Video Thumbnail" class="w-24 h-24 object-cover rounded mt-2">
                    @endif
                </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="button_link" name="button_link" placeholder="Contoh: /ppdb atau https://youtube.com/..."
                    class="w-full border-gray-300 rounded px-3 py-2"
            ...
            <div class="mb-4">
                ...
                <input type="number" name="order" value="{{ old('order', $carousel->order) }}" class="w-full border-gray-300 rounded px-3 py-2">
            </div>

            <div class="mb-4 flex items-center">
                <input type="checkbox" name="is_active" id="is_active" class="mr-2" {{ old('is_active', $carousel->is_active) ? 'checked' : '' }}>
